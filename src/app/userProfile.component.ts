...
@Component({
  ...
  <img *ngIf="DB.getUserPhotoThumb(UI.focusUser)" class="imageWithZoom" [src]="DB.getUserPhotoThumb(UI.focusUser)" style="float:right;object-fit:cover;height:60px;width:60px" (click)="showFullScreenImage(DB.getUserPhotoOriginal(UI.focusUser))">
  </div>
  </div>
