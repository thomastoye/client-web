...
@Component({
  ...
  <div style="color:#888;font-size:11px;padding:5px 5px 5px 10px;clear:both">Joined {{DB.getUserCreatedTimestamp(UI.focusUser)|date:'MMMM yyyy'}}, {{DB.getUserMessageCount(UI.focusUser)==null?0:DB.getUserMessageCount(UI.focusUser)}} Messages, {{DB.getUserTransactionCount(UI.focusUser)==null?0:DB.getUserTransactionCount(UI.focusUser)}} Transactions</div>
  </div>
