...
@Component({
  ...
  <input maxlength="500" (keyup)="refreshProjectList()" [(ngModel)]="this.filter" style="texttransform:uppercase" placeholder="search project name">
    <li *ngFor="let project of projects | async"
      ...
      <img [src]="DB.getProjectImageUrlThumb(project.image)" style="display: inline; float: left; margin: 0 10px 0 10px; opacity: 1; object-fit: cover; height:30px; width:30px">
      {{project.name}}
    </li>
  </ul>
