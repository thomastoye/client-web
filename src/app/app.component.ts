...
@Component({
  ...
  template: `
    ...
    <div style="text-align:center;width:150px;height:40px;cursor:pointer;float:left" (click)="router.navigate(['team',UI.currentTeam])">
        ...
        <img (error)="errorHandler($event)" [src]="DB.getTeamPhotoURL(UI.currentTeam)" style="object-fit:cover;height:15px;width:75px;border-radius:0 0 15px 15px">
        <div style="position:absolute;width:150px;top:17px;text-align:center;color:#fff;border-radius:5px 5px 0 0;font-size:10px;line-height:20px">{{DB.getTeamName(UI.currentTeam)}}{{(DB.getTeamLeader(UI.currentTeam,UI.currentUser)?" *":"")}}</div>
        </div>
    </div>
