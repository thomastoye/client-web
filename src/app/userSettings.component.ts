...
@Component({
  ...
  <ul class="listLight">
    <li style="cursor:default" *ngFor="let team of userTeams | async"
      ...
      <div *ngIf="!DB.getUserLeader(team.$key,UI.focusUser)" class="buttonDiv" style="font-size:11px;color:red;border:none" (click)="db.object('userTeams/'+UI.currentUser+'/'+team.$key).update({following:false})">Stop following</div>
      </div>
